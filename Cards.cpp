...
class node
{
    ...
    void display()
    {
        ...
        printf("(%s,%s,%s)\n", val.c_str(),house.c_str(),colour.c_str());

    }
    node(int val,string House)
    {
        value=val;
        house=House;
        if(house.compare("Clubs")==0||house.compare("Spades")==0)
            colour="Black";
        else if(house.compare("Hearts")==0||house.compare("Diamonds")==0)
            colour="Red";  
        else 
        {
            colour="Please re-enter the House";
        } 
        next=NULL;     
...
void insertAtTail(node* &head,int val,string House)
{
    node* n=new node(val,House);
    if(head==NULL)
    {
        head=n;
        return;
...
node* merge_house(node* &head1,node* &head2)
{
    node* p1=head1;
    node* p2=head2;
    node* dummyNode=new node(0,"zero");
...
int main()
{
    ...
    while(true)
    {
        ...
        else if(input.find("House sort")!=string::npos)
        {
            house_sort(head);
            displaydeck(head);
        }
